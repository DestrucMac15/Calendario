...
class Calendario extends CI_Controller{
	...
	public function save(){
		...
		if(!empty($this->input->post('id'))){
			...
			$encode_dataC = json_encode($json_set);
			...
			if($potentials != Null){
				...
				foreach($potentials['data'] as $potential){
					...
					if($data['Fecha'] == $fecha_sin_t){
						...
						/** SE CREA LA ASOCIACION EN EL MODULO DE 'Calendario' */
						$respuesta = $this->Calendar_model->create_calendar($token,json_decode($encode_dataC))['data'][0];
